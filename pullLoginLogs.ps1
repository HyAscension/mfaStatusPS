...
$timeInSeconds = $timer.TotalSeconds
$timeInSeconds = [Math]::Round($timeInSeconds)

$timeInMinutes = $timeInSeconds / 60
$timeInMinutes = [Math]::Round($timeInMinutes)

if ($timeInMinutes -lt 2) {
    Write-Host "Script execution time: $($timeInSeconds) seconds"
}
else {
    Write-Host "Script execution time: $($timeInSeconds) seconds ($($timeInMinutes) minutes)"
}
